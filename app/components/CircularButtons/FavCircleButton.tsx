...
function FavCircleButton(
   { isFav, onClick, className }: {
      isFav: boolean,
      onClick: Function,
      className: string
   }
) {
   const [isIsFav, setIsisFav] = useState(isFav);
   return (
      <label className={"btn btn-circle swap " + className}>
         <input
            ...
            onClick={
               e => {
                  const newIsIsFav = !isIsFav;
                  setIsisFav(newIsIsFav);
                  onClick(e, newIsIsFav);
               }
            }
         />
         {filledStarIcon}
...
export default FavCircleButton;